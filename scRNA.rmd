...
```{r}
#saveRDS(pbmc, file = paste0(path,"rds_files/seurat_standard.rds"))
```
...
```{r pK2}
sweep.res.list_blood_sct <- paramSweep(pbmc,PCs=1:PC_count,sct=TRUE)
sweep.stats_blood <- summarizeSweep(sweep.res.list_blood)
bcmvn_blood <- find.pK(sweep.stats_blood)
...
```{r}
#saveRDS(pbmc, file = paste0(path,"rds_files/sct_transform.rds"))
```
